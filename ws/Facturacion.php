...

{
    ...
    public function __construct($cliente,$idVenta,$typeDocument,$descuentoGlobal,$medioPago,$tipo_guia,$serie_numero)
    {
    ...
    public function processItemWithSaleData()
    {
        ...
        try{
            ...
            $productos= $this->conection->consulta_matriz("SELECT pv.*,p.incluye_impuesto, p.nombre as descripcion, pt.valor as codigo_producto_sunat FROM producto_venta pv inner join producto p on (pv.id_producto=p.id) left join producto_taxonomiap pt on (p.id=pt.id_producto) WHERE pt.id_taxonomiap=-1 and pv.id_venta = '".$this->idVenta."' GROUP BY pv.id");
            ...
            if(is_array($productos) || is_array($servicios) ){
                if( is_array($productos) ){
                    foreach($productos as $p){
                        ...
                        if($p['incluye_impuesto']==1){//axalpusa
                            ...
                            if($p['descripcion']==$this->ICBPER){
                                     $calculosVenta['total_impuestos_bolsas']=$calculosVenta['total_impuestos_bolsas']+( $this->monto_icbper*$i['cantidad']);
                                     $calculosVenta['total_igv']= $calculosVenta['total_igv']-$i['igv'];
                                     $calculosVenta['total_gravada']= $calculosVenta['total_gravada']-$i['subtotal'];
                                     // $calculosVenta['total_gravada']= $calculosVenta['total_gravada']-$calculosVenta['total_impuestos_bolsas'];
                            }
    ...
    private function calcularIgv($item,$impuestoIn){
        ...
        switch(intval($item['incluye_impuesto'])){
            case 1: // GRAVADA
                ...
                $impuesto= $item['precio']-$valor_unitario;//igv_unitario
                $subtotal=  $valor_unitario*$item['cantidad'];//precio_unitario o precio sin impuesto
    ...
    { 
        // return $cliente;
       /*echo gettype($this->tipo_guia);
       echo " // ";
       echo gettype($this->serie_numero);*/
        ...
        $data = array(
            ...
            "total_impuestos_bolsas"            => $dataVenta['total_impuestos_bolsas'],
            "detraccion"                        => "false",
            "observaciones"                     => "",
            ...
            "formato_de_pdf"                    => "",
            "items"                             => $dataItems,
            //"guias"                             =>json_decode("[{'guia_tipo':".$this->tipo_guia.",'guia_serie_numero':".$this->serie_numero."}]")
        );

        //echo json_encode($dataItems);
    
        //$ff="[{'guia_tipo:'".$this->tipo_guia.",'guia_serie_numero':".$this->serie_numero."}]";
      //  $data["guias"] =json_decode($ff);
      if($this->tipo_guia!=0){
        $data["guias"] = [array(
            'guia_tipo'=>$this->tipo_guia,
            'guia_serie_numero'=>$this->serie_numero
        )
        ];
      }
        ...
        /* $monto_icbper=0.30;
               if(is_array($dataItems)){
                    foreach($dataItems as $i){
                        if($i['descripcion']=='IMPUESTO BOLSA 2021'){
                             $data["total_impuestos_bolsas"] = $monto_icbper*$i['cantidad'];
                        }
               }
            }*/
            ...
            $isDetraccion=false;
             ...
             if(strpos($this->medioPago,"_DETRACCION") !== FALSE){
                  $isDetraccion=true;
             }
             ...
             if( $isDetraccion){
                ...
                $configuracion= $this->conection->consulta_arreglo("SELECT * FROM configuracion ");
                $detraccion= $configuracion["id_detraccion"];

                $query_detraccion= $this->conection->consulta_arreglo("SELECT * FROM porcentaje_detraccion where id = $detraccion");
                        
                $data["detraccion_tipo"] = $query_detraccion['codigo'];
                $data["detraccion_porcentaje"] = floatval($query_detraccion['porcentaje']);
                $data["detraccion_total"] = floatval($query_detraccion['porcentaje'] * $dataVenta['montoTotalVenta'] / 100);
     ...
     //   curl_setopt($ch, CURLOPT_TIMEOUT, 150);
        ...
        if(intval(curl_errno($ch)) === 0){
            ...
            $this->conection->consulta_simple($qrComprobante);
            $return['success']=false;
            $return['message']=['errors'=>'Se produjo un Error con el servidor de SUNAT, RECLAMAR A SUNAT'];
            return $return;
        }
    ...
    public function insertSaleDocument($data,$dataVenta){
        ...
        if($this->typeDocument==0 || $this->typeDocument==3 ){ // NOTA DE VENTA y NOTA DE VENTA CREDITO
            ...
            if($this->typeDocument==0) { // NOTA DE VENTA
               ...
               // echo(" total='".$dataVenta['montoTotalSinDescuento']."'");
                $this->conection->consulta_simple($sql);
            }
